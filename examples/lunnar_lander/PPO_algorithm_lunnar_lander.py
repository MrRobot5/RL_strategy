...
#   Website     : https://pylessons.com/LunarLander-v2-PPO step-by-step reinforcement learning tutorial🎈
...
class Actor_Model:
    def __init__(self, input_shape, action_space, lr, optimizer):
        """
        @param: input_shape for our neural net
        @param: action_space the total number of possible actions available in the current environment and will be the total number of output nodes of the neural net.
        """
        X_input = Input(input_shape)
        self.action_space = action_space

        # I am using three deep superficial layers of Neural Network with the following neurons [512, 256, 64].
        X = Dense(512, activation="relu", kernel_initializer=tf.random_normal_initializer(stddev=0.01))(X_input)
        X = Dense(256, activation="relu", kernel_initializer=tf.random_normal_initializer(stddev=0.01))(X)
        X = Dense(64, activation="relu", kernel_initializer=tf.random_normal_initializer(stddev=0.01))(X)
        # The last layer is the classification layer, with the softmax activation function added on top of this trainable feature extractor layer.
    ...
    def ppo_loss(self, y_true, y_pred):
        """
        Custom PPO loss
        This is the most crucial part of the Proximal Policy Optimization algorithm.

        """
        # Defined in https://arxiv.org/abs/1707.06347
        advantages, prediction_picks, actions = y_true[:, :1], y_true[:, 1:1 + self.action_space], y_true[:, 1 + self.action_space:]
        # we use a clipping parameter LOSS_CLIPPING to ensure we only make the maximum percent change to our policy at a time.
        ...
        p1 = ratio * advantages
        # Using this ratio, we can decide how much of a policy change we are willing to tolerate.
